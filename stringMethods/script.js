...
let quotation = "Nobody calls me Lebowski, you got the wrong guy, I'm the Dude, man."
let log = console.log;
log(quotation)
...
log(quotation.toLowerCase().lastIndexOf('the'))

//substring and substr methods//
let str = quotation;
//substring(from, to) up to but not including
...
if (paragraph.indexOf(find) > -1) {
    let p = paragraph.replace(find, replace);
    log(p)
}
...
//trimming(removal of white space) and padding strings//
//trim() , trimLeft, trimRight,
//paStart(minlength, [padstring]) padEnd()
let name = "    Neil Edward     ";
let wholeName = "Neil Edward Dela Cruz";
...
log(wholeName.padEnd(55, '/-/-/-'))

//rounding and formatting numbers//
let number1 = 1111.467;
let dec1 = 0.000161;
let str1 = "3.14159265359 is the the value of pi"
//Math.round() .floor()-for round down .ceil()- for round up --returns a number
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    text.addEventListener('keydown', upThing);
    //no charcode and input value addded yet
    text.addEventListener('keypress', upThing);
    //charcode no input value added yet
    text.addEventListener('keyup', upThing);
    //no charcode input value added
    text.addEventListener('input', upThing);
    // no charcode but input is availablr or mutable
